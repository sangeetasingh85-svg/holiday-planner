...
def download_pdf(url, timeout=20):
    try:
        ...
        # Warm up session with a head request to the domain
        domain = "/".join(url.split("/")[:3])
        try:
            session.get(domain, headers=HEADERS, timeout=5, allow_redirects=True)
        except Exception:
            pass
        ...
        if resp.status_code == 200:
            ...
            # Confirm it's actually a PDF by checking magic bytes
            if content[:4] == b"%PDF":
                return io.BytesIO(content)
            # Some servers return PDF without correct content-type
            if "pdf" in resp.headers.get("Content-Type", "").lower() and len(content) > 500:
                return io.BytesIO(content)
    except Exception:
        pass
    return None
